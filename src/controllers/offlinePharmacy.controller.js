...
const registerPharmacy = async (req, res) => {
    const { name, location, home_delivery_locations, contact_no, email, license_no } = req.body;
    ...
    res.status(201).json({ message: " Pharmacy Registered Successfully", newPharmacy })
}
...
const getMedicine = async (req, res) => {
    ...
    try {
        ...
        if (!pharmacies || pharmacies.length === 0) {
            return res.status(404).json({ message: "No pharmacies found in the given location." });
        }
        ...
        if (!medicines || medicines.length === 0) {
            return res.status(404).json({ message: "No medicines found in the inventory for the given product and location." });
        }
...
const addMedicine = async (req, res) => {
    const { name, manufacturer, expiry, manufactured, qty, price, pharmacyId } = req.body;
...
const getAllMedicine = async (req, res) => {
    ...
    if (!medicines.length) {
        return res.status(404).json({ error: 'No medicines found for this pharmacy' });
    }
...
module.exports = {
    registerPharmacy, getMedicine, addMedicine, getAllMedicine
}